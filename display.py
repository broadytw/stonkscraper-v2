...
class mainMenu(Menu):
    def __init__(self,master=None):
        ...
        # 'Quit' command
        self.add_command(label='Quit',underline=0,command=self.quit)
    ...
    def changeTheme(self):
        ...
        if currtheme == 'light':
            ...
            currtheme = 'light'
        #showinfo(title='Changing Theme',message=f'currtheme = {currtheme}')
        app.destroy()
        app = tkui()
        app.mainloop()
        #tkui().changeTheme()
...
class mainFrame(Frame):
    def __init__(self,master):
        ...
        master.geometry(f'{display_info[0]}x{display_info[1]}')
        ...
        #fig_scale = display_info[2]*(display_info[2]/(display_info[0]-display_info[1]))
        fig_scale = display_info[2]*(display_info[2]/48)
        fig = Figure(figsize=(display_info[0]/fig_scale, display_info[1]/fig_scale), dpi=display_info[2])
    ...
    # This method below returns the current stock search
...
class tkui(Tk):
    def __init__(self):
        ...
        self.title('Stonk Scraper')                                                 # window title
        self.iconphoto(False, PhotoImage(file='cashmoney.png'))                     # window icon
        self.configure(background=theme[currtheme]['main']['bg'])                   # window background
        display_info = (self.winfo_screenwidth(), self.winfo_screenheight(), self.winfo_fpixels('1i'))  # display_info = (width, height, dpi)
        self.geometry(f'{display_info[0]}x{display_info[1]}')                       # window by screensize
